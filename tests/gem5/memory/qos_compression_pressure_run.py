...
parser = argparse.ArgumentParser(
    description="QoS Compression and Queue Pressure Test"
)
parser.add_argument(
    "--compression-aware",
    action="store_true",
    help="Enable compression awareness",
)
parser.add_argument(
    "--pressure-gradient",
    action="store_true",
    help="Enable pressure gradient turnaround",
)
parser.add_argument(
    "--q-policy",
    type=str,
    default="fifo",
    choices=["fifo", "lifo", "lrg", "cp"],
    help="Queue selection policy",
)
...
turnaround = QoSTurnaroundPolicyIdeal(
    enable_pressure_gradient=args.pressure_gradient, hysteresis_threshold=0.1
)
...
m5.instantiate()
print(
    f"Beginning QoS Test (compression_aware={args.compression_aware}, pressure_gradient={args.pressure_gradient}, q_policy={args.q_policy})..."
)
