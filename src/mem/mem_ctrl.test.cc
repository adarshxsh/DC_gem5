...
TEST(MemCtrlAdaptiveTest, PressureGradientAndEffectiveThreshold)
{
    ...
    uint32_t hard_ceiling =
        (uint32_t)std::floor(write_buffer_size * 0.95); // 60
    ...
    for (double gradient = 0.0; gradient <= 1.0; gradient += 0.25) {
        uint32_t dynamic_high =
            write_high_base +
            (uint32_t)std::round((write_high_max - write_high_base) *
                                 gradient);
...
TEST(MemCtrlAdaptiveTest, DynamicWatermarkFormula)
{
    ...
    double grad = std::max(
        0.0, std::min(1.0, 0.5 * rd_occ + 0.5 * rd_wait - 0.5 * wr_occ));
    EXPECT_NEAR(grad, 0.6, 1e-5);

    uint32_t base_high = 50;
    uint32_t max_high = 60;
    uint32_t eff_high =
        base_high + (uint32_t)std::round((max_high - base_high) * grad);
...
TEST(MemCtrlAdaptiveTest, AdaptiveBurstBatching)
{
    ...
    for (uint32_t write_q_size = 0; write_q_size <= write_buffer_size;
         write_q_size += 16) {
        double wr_occ_ratio =
            std::min(1.0, (double)write_q_size / write_buffer_size);
