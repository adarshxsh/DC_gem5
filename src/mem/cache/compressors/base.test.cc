...
TEST(BaseCompressorTest, SlidingWindowRingBufferSampling)
{
    ...
    // 1. Fill window with 4 compressible (zero) blocks. ZeroCompressor compresses zero block to 0 bits.
    for (int i = 0; i < 4; i++) {
    ...
    // Window now has 2 zero blocks (0 bits) and 2 uncompressible blocks (512 bits each).
    ...
    // Window now has 1 zero block (0 bits) and 3 uncompressible blocks (512 bits each).
    compressor.compress(uncompressible_data, comp_lat, decomp_lat);

    EXPECT_EQ(compressor.getSampledUncompressedBits(), 4 * 512); // 2048
    EXPECT_EQ(compressor.getSampledCompressedBits(), 3 * 512);   // 1536
    EXPECT_DOUBLE_EQ(compressor.getObservedRatio(), 2048.0 / 1536.0); // 1.333...

    // Ratio is 1.333... which is below latency_breakeven_threshold (1.5)!
    // 4. Push 4th uncompressible block. Adaptive bypass should bypass compression.
    auto comp_data = compressor.compress(uncompressible_data, comp_lat, decomp_lat);
    EXPECT_EQ(comp_data->getSizeBits(), 512); // uncompressed size due to bypass
...
TEST(BaseCompressorTest, FallbackToExponentialDecayWhenWindowSizeZero)
{
    ...
    // Sample 2: decay reduces previous (512, 512) to (256, 256), then adds (512, 512) -> (768, 768).
    compressor.compress(uncompressible_data, comp_lat, decomp_lat);
