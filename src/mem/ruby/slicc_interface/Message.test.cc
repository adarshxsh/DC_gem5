...
int
RubySystem::MachineType_base_count(const MachineType &t)
{
    return 1;
}
int
RubySystem::MachineType_base_number(const MachineType &t)
{
    return 0;
}
...
class DummyMessage : public Message
{
  ...
  public:
    DummyMessage(int size = -1) : Message(0, 64, nullptr), payload_size(size)
    {}
...
TEST(MessageTest, DynamicFlitCountAndType)
{
    ...
    RouteInfo route;
    flit f_zero(0, 0, 0, 0, route, num_flits_zero, nullptr, msg_size_zero,
                bit_width, 0);
    ...
    flit f_head(1, 0, 0, 0, route, num_flits_32, nullptr, msg_size_32,
                bit_width, 0);
    EXPECT_EQ(f_head.get_type(), HEAD_);

    flit f_body(1, 2, 0, 0, route, num_flits_32, nullptr, msg_size_32,
                bit_width, 0);
    EXPECT_EQ(f_body.get_type(), BODY_);

    flit f_tail(1, 4, 0, 0, route, num_flits_32, nullptr, msg_size_32,
                bit_width, 0);
