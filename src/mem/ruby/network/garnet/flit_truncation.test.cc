 ...
 * All rights reserved.
 *
 * Dynamic Flit Truncation and Compressed Lock Duration Unit Test
 */
...
TEST(FlitTruncationTest, Flit3BitCompressedLengthEncoding)
{
    RouteInfo route;
    route.vnet = 0;
    route.src_ni = 0;
    ...
    flit f0(1, 0, 0, 0, route, 2, msg, 24, 16, 0);

    // 16 bytes payload / 8 = 2 -> 3-bit encoded value is 2
    EXPECT_EQ(f0.get_compressed_length(), 2);

    f0.set_compressed_length(7);
    EXPECT_EQ(f0.get_compressed_length(), 7);

    // Test masking to 3 bits (0x7)
    f0.set_compressed_length(15); // 15 & 7 = 7
    EXPECT_EQ(f0.get_compressed_length(), 7);
}
...
TEST(FlitTruncationTest, FlitSequenceTypesAndSize)
{
    ...
    // Test 2-flit sequence (truncated to 24 bytes: 8 header + 16 compressed
    // payload)
    ...
    // Test 1-flit sequence (zero block payload: 8 header + 0 compressed
    // payload)
    int trunc_size_8 = 8;
...
{
    auto calc_msg_size = [](Message *net_msg_ptr, int uncomp_data_size,
                            int header_size) {
        ...
        if (net_msg_ptr->getCompressedPayloadSize() >= 0 &&
            ...
            int comp_payload = std::min(
                net_msg_ptr->getCompressedPayloadSize(), uncomp_payload);
    ...
    int size1 = calc_msg_size(&uncomp_data_msg, data_msg_uncomp_bytes,
                              control_msg_bytes);
    EXPECT_EQ(size1, 72);
    ...
    int size2 = calc_msg_size(&comp16_data_msg, data_msg_uncomp_bytes,
                              control_msg_bytes);
    ...
    int size3 = calc_msg_size(&comp0_data_msg, data_msg_uncomp_bytes,
                              control_msg_bytes);
