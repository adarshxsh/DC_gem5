...
def get_metric(
    stats: Dict[str, float], keys: list, default: float = 0.0
) -> float:
    for k in keys:
        if k in stats:
            return stats[k]
...
def print_comparison(
    baseline_stats: Dict[str, float], bdi_stats: Dict[str, float]
):
    print("=" * 80)
    print(
        f"{'Metric':<45} | {'Baseline':<14} | {'BDI Compressed':<14} | {'Delta (%)':<10}"
    )
    print("=" * 80)
    ...
    base_ipc = get_metric(
        baseline_stats,
        ["board.processor.cores.core.ipc", "sim_insts / sim_ticks", "ipc"],
    )
    bdi_ipc = get_metric(
        bdi_stats,
        ["board.processor.cores.core.ipc", "sim_insts / sim_ticks", "ipc"],
    ...
    base_cpi = get_metric(
        baseline_stats, ["board.processor.cores.core.cpi", "cpi"]
    )
    ...
    sim_seconds_base = get_metric(
        baseline_stats, ["simSeconds", "sim_seconds"]
    )
    ...
    def format_row(
        ...
        if is_ratio:
            print(
                f"{name:<45} | {base_val:<14.4f} | {bdi_val:<14.4f} | {delta_str:<10}"
            )
        else:
            print(
                f"{name:<45} | {base_val:<14.0f} | {bdi_val:<14.0f} | {delta_str:<10}"
            )

    print("--- Core Execution ---")
    format_row(
        "Simulated Instructions (ROI)", sim_insts_base, sim_insts_bdi, False
    )
    ...
    print("\n--- L1 Caches ---")
    base_l1d_misses = get_metric(
        baseline_stats,
        [".*l1d.*overallMisses::total", ".*l1d-cache.*demandMisses::total"],
    )
    bdi_l1d_misses = get_metric(
        bdi_stats,
        [".*l1d.*overallMisses::total", ".*l1d-cache.*demandMisses::total"],
    )
    base_l1d_mshr = get_metric(
        baseline_stats, [".*l1d.*overallMshrMisses::total"]
    )
    bdi_l1d_mshr = get_metric(bdi_stats, [".*l1d.*overallMshrMisses::total"])
    format_row(
        "L1D Overall Misses", base_l1d_misses, bdi_l1d_misses, False, False
    )
    format_row("L1D MSHR Misses", base_l1d_mshr, bdi_l1d_mshr, False, False)

    print("\n--- L2 Cache ---")
    base_l2_accesses = get_metric(
        baseline_stats,
        [".*l2.*overallAccesses::total", ".*l2-cache.*demandAccesses::total"],
    )
    bdi_l2_accesses = get_metric(
        bdi_stats,
        [".*l2.*overallAccesses::total", ".*l2-cache.*demandAccesses::total"],
    )
    base_l2_misses = get_metric(
        baseline_stats,
        [".*l2.*overallMisses::total", ".*l2-cache.*demandMisses::total"],
    )
    bdi_l2_misses = get_metric(
        bdi_stats,
        [".*l2.*overallMisses::total", ".*l2-cache.*demandMisses::total"],
    )
    base_l2_miss_rate = get_metric(
        baseline_stats,
        [".*l2.*overallMissRate::total", ".*l2-cache.*demandMissRate::total"],
    )
    bdi_l2_miss_rate = get_metric(
        bdi_stats,
        [".*l2.*overallMissRate::total", ".*l2-cache.*demandMissRate::total"],
    )
    ...
    format_row(
        "L2 Miss Rate", base_l2_miss_rate, bdi_l2_miss_rate, True, False
    )

    print("\n--- Compression Metrics (BDI) ---")
    bdi_comp_ratio = get_metric(
        bdi_stats,
        [
    ...
    bdi_comp_blks = get_metric(
        bdi_stats,
        [
            ".*compressor.*compressed_blocks",
            ".*compressor.*total_compressed_blocks",
        ],
    )
    bdi_uncomp_blks = get_metric(
        bdi_stats, [".*compressor.*uncompressed_blocks"]
    )
    print(
        f"{'Compression Ratio':<45} | {'1.0000 (N/A)':<14} | {bdi_comp_ratio:<14.4f} | {'N/A':<10}"
    )
    print(
        f"{'Compressed Cache Blocks':<45} | {'0':<14} | {bdi_comp_blks:<14.0f} | {'N/A':<10}"
    )
    print(
        f"{'Uncompressed Blocks':<45} | {'N/A':<14} | {bdi_uncomp_blks:<14.0f} | {'N/A':<10}"
    )

    print("\n--- Memory Subsystem Traffic ---")
    base_dram_read = get_metric(baseline_stats, [".*dramBytesRead$"])
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python3 parse_compression_stats.py <m5out_baseline/stats.txt> <m5out_bdi/stats.txt>"
        )
        sys.exit(1)
